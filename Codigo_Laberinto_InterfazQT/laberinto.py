...
# Módulos asociados al serial
import serial #dwddl
import serial.tools.list_ports as list_ports
from time import sleep

# Módulos asociados al seguimiento de objetos: 
from collections import deque
import numpy as np
import cv2


# Define los parámetros de color que detectará
greenLower = (0, 192, 0)
greenUpper = (255, 255, 195)
pts = deque(maxlen=64)
        ...
        while True:
            if self.activarCamara == True:
                if self.cam.isOpened():
                    ...
                    mask = cv2.inRange(magenHSV, greenLower, greenUpper)  # Recibe los parámetros de color
                    mask = cv2.erode(mask, None, iterations=2)
                    mask = cv2.dilate(mask, None, iterations=2)

# - - - - - - - - - - Encontrar el contorno de la bola  y fijar el centro (x, y) - - - - - - - - - #
                    cnts = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL,
                        cv2.CHAIN_APPROX_SIMPLE)[-2]
                    center = None
                    
                    # Sólo procede si encuentra al menos un contorno
                    if len(cnts) > 0:
                        # Encuentra el contorno más largo en mask
                        
                        # Luego lo usa y computa el mínimo circulito y centro
                        c = max(cnts, key=cv2.contourArea)
                        ((x, y), radius) = cv2.minEnclosingCircle(c)
                        M = cv2.moments(c)
                        center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))

                        # Sólo procede si el radio conoce un tamaño mínimo
                        if radius > 10:
                            cv2.circle(frame, (int(x), int(y)), int(radius), # Dibuja el círculo
                                (0, 255, 255), 2)
                            cv2.circle(frame, center, 5, (0, 0, 255), -1) # Actualiza puntos trazados
                        
                    # Actualiza lista de puntos
                    pts.appendleft(center)

                    # bucle sobre el conjunto de puntos rastreados
                    for i in range(1, len(pts)):
                        # Si cualquier de los puntos rastreados no es del color...
                        # ignórelo.
                        if pts[i - 1] is None or pts[i] is None:
                            continue

                        # Computa el grosor de la línea y...
                        # dibuja los puntos conectados
